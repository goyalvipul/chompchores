...
| Host | What you do |
|---|---|
| **Cloudflare Pages** | Connect Git → **Build command** `npm run pwa:config` → **Output** `apps/web-pwa`. Set env vars `SUPABASE_URL` + `SUPABASE_ANON_KEY` (writes gitignored `config.js`). Details in [`apps/web-pwa/README.md`](../apps/web-pwa/README.md). |
